 ...
 * Each returns { subject, html, text }. Pass the recipient's first name (or a
 * warm fallback) for personalization. Wire-up example in ./send.ts.
 */

export type EmailContent = { subject: string; html: string; text: string };

/** A warm, brand-safe greeting that never reads "Hi undefined". */
...
export function foundingWelcomeEmail(opts: { name?: string | null }): EmailContent {
  ...
  const html = emailShell({
    ...
    bodyHtml:
      p(`Thank you, ${who}. Your founding seat is locked in — a lifetime seat at the lowest price ${siteConfig.name} will ever offer, and a price that never moves.`) +
  ...
  const text = [
    "Welcome, Founder.",
    "",
    `Thank you, ${who}. Your founding seat is locked in — a lifetime seat at the lowest price ${siteConfig.name} will ever offer, and a price that never moves.`,
    "",
...
export function waitlistConfirmationEmail(opts: { name?: string | null }): EmailContent {
  ...
  const html = emailShell({
    ...
    bodyHtml:
      p(`Thank you for joining, ${who}. We'll write to you as the first city draws closer.`) +
  ...
  const text = [
    "You're on the list.",
    "",
    `Thank you for joining, ${who}. We'll write to you as the first city draws closer.`,
    "",
...
export function verifyEmail(opts: { name?: string | null; verifyUrl: string }): EmailContent {
  ...
  const html = emailShell({
    ...
    bodyHtml:
      p(`Welcome, ${who}. Confirm your email and the room is yours — anonymous until you decide otherwise.`) +
  ...
  const text = [
    "Step inside.",
    "",
    `Welcome, ${who}. Confirm your email and the room is yours.`,
    "",
